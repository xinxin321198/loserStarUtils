package com.loserstar.utils.password;
 ...
 *
 */
public class LoserStarPwdCheckUtil {
	 ...
	 * @return 
	 */  
	 static String removeAllBlank(String s){  
	    String result = "";  
	    if(null!=s && !"".equals(s)){  
	...
	public static boolean isPwdOk(String pwd){
		...
		if( checkPasswordLength(pwd,"8","16")==false) return false;
		
		
		if( pwd.contains("　"))return false ;//不能包含空白字符
		if(removeAllBlank(pwd).equals(pwd)==false) return false;//不能包含空白字符
		int i=0;
		if(checkContainDigit(pwd)){
			i++;
		}
		if(checkContainCase(pwd)){
			i++;
		}
		if(checkContainSpecialChar(pwd)){
			i++;
		}
		return i>1;
	}
   ...
   public static boolean checkPasswordLength(String password, String minNum, String maxNum) {
       ...
       if (maxNum==null||maxNum.equals(""))  {
       	minNum = minNum==null||minNum.equals("") == true ? "0":minNum;
