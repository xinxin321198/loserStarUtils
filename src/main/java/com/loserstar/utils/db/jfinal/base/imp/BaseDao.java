...
public abstract class BaseDao {
	...
	/**
	 * 查询列表(分页)的列表数据（参数化查询，并转为对应实体类） new一个whereHelper参数：如果设置过软删除字段自动过滤 null:直接不添加软删除过滤
	 * 
	 * @param pageNumber
	 *            页码
	 * @param pageSize
	 ...
	 * @return
	 */
	public <T extends Model<T>> Page<T> getListPage(int pageNumber, int pageSize, WhereHelper whereHelper, Class<T> class1, Object[] values) {
		addSoftDelField(whereHelper);
		return getListPage_notSoftDel(pageNumber, pageSize, whereHelper, class1, values);
	}
	 ...
	 * @return
	 */
	public Page<Record> getListPage(int pageNumber, int pageSize, WhereHelper whereHelper, Object[] values) {
		addSoftDelField(whereHelper);
		return getListPage_notSoftDel(pageNumber, pageSize, whereHelper, values);
	}
	 ...
	 * @return
	 */
	public <T extends Model<T>> Page<T> getListPage_notSoftDel(int pageNumber, int pageSize, WhereHelper whereHelper, Class<T> class1, Object[] values) {
		Page<T> page = null;
